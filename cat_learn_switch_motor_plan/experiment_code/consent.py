...
test = '''We are seeking healthy volunteers with normal or corrected-to-normal vision and normal hearing to participate in a study that investigates the cognitive mechanisms underlying visual 
and/or auditory processing, human sensorimotor learning, processing of body-related information and how such information modulates perception and action.
...
requirements of Bachelor of Psychology (Honours) under the supervision of Matthew Crossley (telephone (02) 9850 2970; email matthew.crossley@mq.edu.au) and David Kaplan 
(telephone (02) 9850 2943; email david.kaplan@mq.edu.au) of the School of Psychological Sciences.

If you decide to participate, you will be asked to view and respond to novel visual stimuli, involving sinewave gratings that will be displayed on a computer screen. You may be asked 
to provide button-press responses during the session. The responses you make, and the timing associated with your responses will be collected. The experimental session may take 
up to 60 minutes to complete, and no risks are expected to result from participation.
...
publication of the results. Access to identifiable data is limited to persons listed on this consent form. Your individual de-identified (anonymised) data obtained through this research 
may be used in future research publications and be made available to journals/reviewers to support publications, as well as in online data repositories, such as the Open Science 
...
By agreeing below, I acknowledge that I have read (or, where appropriate, have had read to me) and understand the information above and any questions I have asked have 
been answered to my satisfaction. I agree to participate in this research, knowing that I can withdraw from further participation in the research at any time without consequence. 
...
def validate_fields(values):
    if values[0].strip() == "" or values[1].strip() == "":
        return False
    else: 
        return True 
...
while True: 
    ...
    if myDlg.OK: 
        if validate_fields(ok_data):
            name_input = ok_data[0]
            email_input = ok_data[1]
            print('user consented')
            break
        else:
            myDlg.addText('Please ensure to fill in all response fields')
...
import csv
f_okaydata = open('../data/picf_data.csv', 'a')
okaydata = csv.writer(f_okaydata, quoting=csv.QUOTE_MINIMAL)
okaydata.writerow([x for x in ok_data])
f_okaydata.close()
