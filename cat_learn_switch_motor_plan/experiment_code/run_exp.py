...
# stimuli
cue_stim = visual.Rect(win, width=15, height=15, fillColor='gray', ori=0)
...
if np.unique(condition).shape[0] > 1:
    ...
    if np.unique(condition) == 'fingers':
        resp_keys = ['d', 'k', 'escape']
    elif np.unique(condition) == 'buttons':
...
while current_trial < num_trials:
    ...
    if cue[current_trial] == 1:
        cue_stim.ori = 45
    elif cue[current_trial] == 2:
        cue_stim.ori = 0
    ...
    if state == 'stim':
        ...
        if len(resp) > 0:
            if len(resp) > 1:
                # TODO: This should transition to a new state and then transitition back without incrementing to the next trial
                # TODO: The new state should draw this message instead of merely printing to the console
    ...
    if state == 'iti':
        if state_clock.getTime() > t_iti:
            ...
            trial_record['cue'].append(cue[current_trial])
