...
def fisherdiscrim(subjdata):
    ...
    kpooled = ((alength - 1) * ka +
               (blength - 1) * kb) / (alength + blength - 2)
...
def get_params_from_alpha(alpha, data):
    ...
    # 1d case.
    b = np.array(
        np.linalg.solve(np.matrix(alpha * acov + (1 - alpha) * bcov),
                        np.matrix(bmu - amu).T)).ravel()
...
class onedfit(modelfit):
    ...
    def calcnegloglike(self):
        ...
        for dim in range(len(self.data[0]) - 1):
            ...
            print "Sigma^2 inaccuracy: ", xopt[0] - var_from_b(
                xopt[1:-1], self.data)
        ...
        self.results = dict(logliks=logliks, ps=params)
        thisxy = np.zeros(2)
...
def compare_fits(fns):
    models = [
        onedfit, twodfit, onedfit_alpha, twodfit_alpha, onedfit_fisher,
        twodfit_fisher
    ]
    modelnames = [
        "onedfit", "twodfit", "onedfit_alpha", "twodfit_alpha",
        "onedfit_fisher", "twodfit_fisher"
    ]
    results = {}
    for fn in fns:
...
def test(fn, twod=True, dim=0):
    ...
    if twod:
        ...
        #b, c0 = get_params_from_alpha( alpha, data )
        import pylab as pl
        plotline(*alpha_params[1:],
                 linestyle='--',
                 color='k',
                 label="Result of alpha opt")
