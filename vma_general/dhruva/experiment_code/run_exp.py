...
cursor_cloud = [visual.Circle(win, radius=0.25, fillColor="white")] * 50
...
config = pd.read_csv(
    "../config/config_reach_" + str(sub_num) + ".csv", na_values="none"
)
...
while current_trial < num_trials:
    ...
    if state == "trial_init":
        ...
        # sig_mp and sig_ep are used later to check for infinite uncertainty
        sig_mp = cursor_mp_sig[current_trial]
        sig_ep = cursor_ep_sig[current_trial]

        # infinite uncertainty trials are equivalent to no-feedback trials
        if sig_mp == -1:
            midopint_vis = 0
            ...
            cursor_cloud_jitter_mp = np.random.multivariate_normal(
                [0, 0],
                [[np.sqrt(sig_mp), 0], [0, np.sqrt(sig_mp)]],
                len(cursor_cloud),
            )
        ...
        if sig_ep == -1:
            ...
            cursor_cloud_jitter_ep = np.random.multivariate_normal(
                [0, 0],
                [[np.sqrt(sig_ep), 0], [0, np.sqrt(sig_ep)]],
                len(cursor_cloud),
            )
    ...
    if state == "instruct":
        if instruct_phase[current_trial] != "none":
            text_stim.text = instruct_phase[current_trial]
            text_stim.draw()
    ...
    if state == "move_prep":
        if instruct_state[current_trial]:
            text_stim.text = (
                "Slice through the target as quickly and accurately as possible"
            )
            text_stim.draw()
    ...
    if state == "reach":
        ...
        if mathtools.distance(start_circle.pos, (x, y)) >= target_distance:
            ...
            endpoint_theta = coordinatetools.cart2pol(
    ...
    if state == "feedback":
        if movement_time > t_too_slow:
            ...
            if endpoint_vis[current_trial]:
                if instruct_state[current_trial]:
                    text_stim.text = (
                        "The on screen cursor shows you how accurate your reach was"
                    )
                    text_stim.draw()
                ...
                if instruct_state[current_trial]:
                    ...
                    text_stim.text += (
                        "so you do not get to see how accurate your reach was."
                    )
    ...
    if state == "iti":
        ...
        if state_clock.getTime() > t_iti:
            ...
            trial_data = {
                ...
                "cycle_phase": [cycle_phase[current_trial]],
            ...
            # only advance to next trial if movement wasn't too fast or too
            # slow.
            if (t_slow == False) and (t_fast == False):
                current_trial += 1
